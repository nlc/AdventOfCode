...
%w{ sample_31.txt sample_165.txt sample_13312.txt sample_180697.txt sample_2210736.txt }.each do |fname|
  ...
  surplus.each do |type, amount|
    ...
    localsurplus = {}
    bolusbelow = footprint(products, (amount / bundle) * bundle, type, localsurplus).first

    # do it again
    localsurplus.each do |localtype, localamount|
      localbundle = products[localtype].keys.first

      localsurplus = {}
      localbolusbelow = footprint(products, (localamount / localbundle) * localbundle, localtype).first
      bolusbelow -= localbolusbelow
    end
  ...
  print "#{target} - #{fp} = #{target - fp}"
  if target - fp < 0
    print ' (redundancy was left over)'
  elsif target - fp > 0
    print ' (too many redundancies removed)'
  end
  puts
end
